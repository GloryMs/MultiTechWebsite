...
const VALUES = [
  {
    ...
    en: { title: 'Reliability', desc: 'Delivering on our commitments with consistency and precision.' },
    ar: { title: 'الموثوقية', desc: 'الوفاء بالتزاماتنا بثبات ودقة.' },
  },
  {
    icon: (
      <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    en: { title: 'Customer-Centricity', desc: 'Putting client needs at the heart of every solution we design.' },
    ar: { title: 'التركيز على العميل', desc: 'وضع احتياجات العميل في صميم كل حل نصممه.' },
  },
  {
    icon: (
      <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    en: { title: 'Continuous Improvement', desc: 'Constantly evolving our processes, skills, and technologies.' },
    ar: { title: 'التحسين المستمر', desc: 'تطوير عملياتنا ومهاراتنا وتقنياتنا باستمرار.' },
  },
  {
    icon: (
      <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    en: { title: 'Innovation', desc: 'Embracing new technologies to deliver smarter, future-proof solutions.' },
    ar: { title: 'الابتكار', desc: 'تبني التقنيات الجديدة لتقديم حلول أذكى ومستدامة.' },
  },
];
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-12 lg:grid-cols-2 items-center">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <div className="mt-2 text-white/60 text-lg">
                        {locale === 'ar' ? 'سنة من الخبرة' : 'Years of Excellence'}
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                <p className="text-brand-text leading-relaxed text-lg mb-4">
                  {locale === 'ar'
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-8 md:grid-cols-2">
              ...
              >
                ...
                <p className="text-brand-text text-lg leading-relaxed">{t('missionText')}</p>
              </motion.div>
            </div>
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-center text-3xl font-bold text-brand-primary mb-16">
              {locale === 'ar' ? 'رحلتنا عبر السنين' : 'Our Journey Through the Years'}
            </h2>
            ...
            <div className="relative">
              ...
              {TIMELINE.map((item, i) => {
                const content = locale === 'ar' ? item.ar : item.en;
                const isEven = i % 2 === 0;
                return (
                  <motion.div
                    key={item.year}
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      <div className="rounded-2xl bg-white p-6 shadow-lg shadow-brand-primary/5 border border-brand-text/5">
                        ...
                        <h3 className="mt-1 text-lg font-bold text-brand-primary">{content.title}</h3>
                        <p className="mt-2 text-sm text-brand-text leading-relaxed">{content.desc}</p>
                      </div>
                    </div>
                  </motion.div>
                );
        ...
        <section className="bg-white py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {VALUES.map((value, i) => {
                const content = locale === 'ar' ? value.ar : value.en;
                return (
                  <motion.div
                    key={i}
                    initial={{ opacity: 0, y: 30 }}
                  ...
                  >
                    ...
                    <h3 className="text-lg font-bold text-brand-primary">{content.title}</h3>
                    <p className="mt-2 text-sm text-brand-text">{content.desc}</p>
                  </motion.div>
                );
              })}
            </div>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
              {TEAM.map((member, i) => {
                const content = locale === 'ar' ? member.ar : member.en;
                return (
                  <motion.div
                    key={i}
                    initial={{ opacity: 0, y: 30 }}
                  ...
                  >
                    ...
                    <h3 className="text-lg font-bold text-brand-primary">{content.name}</h3>
                    <p className="mt-1 text-sm text-brand-accent font-medium">{content.role}</p>
                  </motion.div>
                );
              })}
            </div>
